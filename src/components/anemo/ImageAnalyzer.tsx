...
import { UploadCloud, Bot, User, CheckCircle, XCircle, FileImage, Send, FileText, Loader2, Sparkles, Activity } from 'lucide-react';
...
export function ImageAnalyzer() {
  ...
  const renderContent = () => {
    switch (analysisStep) {
      case 'idle':
        return (
          <div ref={dropzoneRef} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} className="text-center p-8 border-2 border-dashed rounded-lg transition-colors">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>{imageDescription || 'Analyzing your image...'}</CardDescription>
              </CardHeader>
              <CardContent>
                {analysisStep === 'analyzingImage' && <div className="flex justify-center items-center h-64"><Loader2 className="h-8 w-8 animate-spin" /> <p className='ml-2'>Analyzing...</p></div>}
                {imageUrl && analysisStep !== 'analyzingImage' && (
            ...
            {analysisStep === 'interview' && (
                ...
                <CardContent className="flex-1 flex flex-col overflow-hidden">
                    ...
                    <form onSubmit={handleInterviewSubmit} className="flex items-center gap-2 border-t pt-4">
                        ...
                        <Button type="submit" disabled={!userInput.trim() || isInterviewComplete || isInterviewLoading}><Send size={16} /></Button>
                    </form>
                </CardContent>
                </Card>
            )}
